                    ...
                    <table>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>해결일 <i class="ic_sort"></i></th>
                                <th>상세 정보</th>
                            </tr>
                        </thead>

                        <tbody>
                            <tr v-for="item in itemList" v-bind:key="'defect' + item.faultId">
                                <td>
                                    <span class="checkLabel">
...
export default {
    name: "DefectHistoryList",
    components: { DefectDetailModal },
    props: [],
    data: function() {
        return {
            /* selected variable */
    ...
    methods: {
        ...
        search: function() {
            ...
            if(diff < 0) {
                alert.openAlert("고장 이력", "시작일이 종료일보다 빠르게 설정되었습니다.\n시작일과 종료일을 동일하게 맞추어 조회합니다.");''

                this.elStartDate = this.elEndDate;
                params.startDate = params.endDate;
            }

            console.log("[ API ]  : 고장 이력 리스트 조회 (/sms/maintenance/getFaultHistoryList)", params);
            ...
            axios.getApi('/sms/maintenance/getFaultHistoryList', params).then(response => {
                ...
                var errCode = error.response.status;
                if(errCode >= 400) {
                    alert.openAlert("고장 이력", "[ errorCode : " + errCode + " ]\n오류가 발생하였습니다.");''
                }
            });
        },
        ...
        toExcel: function() {
            let alert = this;
            alert.openAlert("고장 이력", "엑셀 내보내기 기능");
        }
    },
    computed: {
        checkAll: {
            ...
            set: function (value) {
                ...
                this.checkedCountText = (this.checked.length > 0) ? this.checked.length + '개의 디바이스가 선택되었습니다.' :  '선택된 디바이스가 없습니다.';
            }
        }
    },
    created() {
