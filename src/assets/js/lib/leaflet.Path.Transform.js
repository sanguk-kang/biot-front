...
L.Handler.PathTransform = L.Handler.extend({
  ...
  _update: function() {
    ...
    for (var i = 0, len = this._handlers.length; i < len; i++) {
      ...
      if (handler !== this._originMarker) {
		...
		if(!handler._updatePath && handler.update) handler.update();
        else handler._updatePath();
      }
    }
  ...
  _apply: function() {
    ...
    for (var i = 0, len = this._handlers.length; i < len; i++) {
	  ...
	  if(!handler.redraw && handler.update) handler.update();
      else handler.redraw();
    }
  ...
  _cachePoints: function() {
    ...
    for (var i = 0, len = this._handlers.length; i < len; i++) {
      var handler = this._handlers[i];
	  if(!handler._point && handler._icon){
		  handler._point = L.DomUtil.getPosition(handler._icon);
	  }
  ...
  _getBoundingPolygon: function() {
    /*if (this._rectShape) {
	  ...
	  //console.error(this._path);
	  //console.error(this._path.getBounds());
