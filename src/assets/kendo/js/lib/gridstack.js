...
(function(factory) {
    ...
    */
    ...
    GridStackEngine.prototype._notify = function() {
		...
		var deletedNodes = args[0].concat(this.nodes);
    ...
    var GridStack = function(el, opts) {
        ...
        if (!self.opts.staticGrid && self.opts.acceptWidgets) {
            ...
            var onDrag = function(event, ui) {
                ...
                if (!node._added) {
                    var isFit = self.isAllowedAdd(x, y, node.width, node.height);
                    console.info("onDrag isFit :"+isFit);
                    el.data('_isFit', isFit);
                    if(!isFit){
                        return;
                    }
                    node._added = true;
            ...
            this.dd
                ...
                .on(self.container, 'drop', function(event, ui) {
                    ...
                    var isFit = $(ui.draggable).data('_isFit');
                    if(!node){
                        console.log("drop event arguments");
                        console.log(event);
                        console.log(ui);
                        //self.container.trigger('dropover', event, ui);
                        self.container.trigger('dropover', ui);
                        //self.container.trigger('drop', event, ui);
                        return false;
                    }
                    if(!isFit){
                        console.info("drop isFit :"+isFit);
                        //self.container.trigger('dropover', event, ui);
                        //$(ui.draggable)
                        return false;
                    }
    ...
    GridStack.prototype._prepareElementsByNode = function(el, node) {
        ...
        var dragOrResize = function(event, ui) {
            ...
            //var y = Math.floor((ui.position.top + 78/2) / 78);
            //console.log("new y : "+y);
            //실제 카드 Cell 사이즈로 y값 측정되도록 보정
            var realCellHeight = 81.5;
            var realCellWidth = 162;
            
            var y = Math.floor((ui.position.top + realCellHeight/2) / realCellHeight);
            //console.log("new y 2: "+y);
            //console.log("ui position.top ; " +ui.position.top + ", cellHeight :"+cellHeight+", calc :"+((ui.position.top + cellHeight / 2) / cellHeight));
            var width;
            var height;
            ...
            if (event.type != 'drag') {
                ...
                width = Math.round(ui.helper.width() / realCellWidth);
                height = Math.round(ui.helper.height() / realCellHeight);
            }
            //console.log("width:"+width );
            //console.log("height:"+height );
            if(x < 0 || y < 0 || ((x+width) > self.grid.width) || ((y+height) > self.grid.height)){
            ...
            if (event.type == 'drag') {
                if (x < 0 || x >= self.grid.width || y < 0 || (!self.grid.float && y > self.grid.getGridHeight())) {
                    /*console.log(x <0);
                    console.log(x >= self.grid.width);
                    console.log(y <0);
                    console.log((!self.grid.float && y > self.grid.getGridHeight()));*/
            ...
            //if(!self.isAllowedAdd(x, y, width, height)){
            //if(!self.willItFit(x, y, width, height)){
            //if(!self.isAllowedReplace(x, y, width, height)){
            //    return;
            //}
            node.lastTriedX = x;
            node.lastTriedY = y;
    ...
    GridStack.prototype.isAllowedAdd = function(x, y, width, height, autoPosition){
        var node = {x: x, y: y, width: width, height: height, autoPosition: autoPosition};    
        var self = this.grid;
        console.info(self.height);
        if (!self.height) {
            return true;
        }
        console.info(self.height);
        ...
        var clone = new GridStackEngine(
            ...
            self.float,
            self.height,
            _.map(self.nodes, function(n) { return $.extend({}, n); }));
        clone.addNode(node);
        console.log("node :"+x+", "+y+" / "+width+", "+height);
        console.log("grid height ->"+clone.getGridHeight());
        console.log("this height -> "+self.height);
        return clone.getGridHeight() <= self.height;
    };
    
    
    GridStack.prototype.isAllowedReplace = function(x, y, width, height, autoPosition){
        var node = {x: x, y: y, width: width, height: height, autoPosition: autoPosition};    
        var self = this.grid;
        console.info(self.height);
        if (!self.height) {
        ...
        console.log("grid height ->"+clone.getGridHeight());
        console.log("this height -> "+self.height);
        return clone.getGridHeight()-height <= self.height;
    };
    ...
    GridStack.prototype.is_allowed_add = obsolete(GridStack.prototype.isAllowedAdd,
        'is_allowed_add', 'isAllowedAdd');
    GridStack.prototype.is_allowed_replace = obsolete(GridStack.prototype.isAllowedReplace,
        'is_allowed_replace', 'isAllowedReplace');
