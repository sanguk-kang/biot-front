<template>
  <sweet-modal ref="ChangePasswordModal" overlay-theme="dark" id="sweet_modal_style">
    <!-- 모달 기본 스타일 시작 -->
    <!-- 모달 제목 영역 S-->
    <div class="sweet_modal_title">비밀번호 입력</div>

    <!-- DAFT : 작업영역 시작 -->
    <!-- 세로 가운데 정렬 일때 flex_al_center -->
    <br /><h3 class="find-contents-title">비밀번호 입력</h3>
        ...
        <ul class="find-idpw-button">
          ...
          <li>
            <button
              ...
              @click="openModalName()"
            >변경하기</button>
            <ConfirmChangingPasswordModal ref="ConfirmChangingPasswordModal"></ConfirmChangingPasswordModal>
          </li>
        </ul>
      </fieldset>
    </form>
...
import ConfirmChangingPasswordModal from "@/views/common/sign-in/ConfirmChangingPasswordModal";

export default {
  name: "ChangePasswordModal",

components: {
  ConfirmChangingPasswordModal
},
  ...
  methods: {
    ...
    openModalName() {
      this.$refs.ConfirmChangingPasswordModal.openModal();
      // this.$refs.ChangePasswordModal.closeModal();
    ...
    saveChangedPasswordAndReturnToSignInPage() {
      ...
      axios.postApi("/ums/user/" + param.userId + "/password", param, "") .then(res => {
